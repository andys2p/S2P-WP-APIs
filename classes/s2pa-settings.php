...
if ( ! class_exists( 'S2PA_Settings' ) ) {

	/**
	 * Handles plugin settings and user profile meta fields
	 */
	class S2PA_Settings extends S2PA_Module {
		protected $settings;
		 ...
		 * @param array  $value This will be merged with S2PA_Settings->settings, so it should mimic the structure of the S2PA_Settings::$default_settings. It only needs the contain the values that will change, though. See Smart2Pay_API_Plugin->upgrade() for an example.
		 */
		public function __set( $variable, $value ) {
			// Note: S2PA_Module::__set() is automatically called before this
			...
			update_option( 's2pa_settings', $this->settings );
		}
		...
		protected static function get_settings() {
			$settings = shortcode_atts(
				...
				get_option( 's2pa_settings', array() )
			);
		...
		public static function add_plugin_action_links( $links ) {
			array_unshift( $links, '<a href="http://wordpress.org/extend/plugins/smart2pay-api-plugin/faq/">Help</a>' );
			array_unshift( $links, '<a href="options-general.php?page=' . 's2pa_settings">Settings</a>' );
		...
		public static function markup_settings_page() {
			if ( current_user_can( self::REQUIRED_CAPABILITY ) ) {
				echo self::render_template( 's2pa-settings/page-settings.php' );
			} else {
		...
		public function register_settings() {
			...
			add_settings_field(
				...
				's2pa_settings',
				's2pa_section-advanced',
				array( 'label_for' => 's2pa_field-example2' )
			);
		...
		public static function markup_section_headers( $section ) {
			echo self::render_template( 's2pa-settings/page-settings-section-headers.php', array( 'section' => $section ), 'always' );
		}
		...
		public function markup_fields( $field ) {
			switch ( $field['label_for'] ) {
				case 's2pa_field-example1':
					// Do any extra processing here
					break;
			}

			echo self::render_template( 's2pa-settings/page-settings-fields.php', array( 'settings' => $this->settings, 'field' => $field ), 'always' );
		}
		...
		public function validate_settings( $new_settings ) {
			...
			if ( ! is_string( $new_settings['db-version'] ) ) {
				$new_settings['db-version'] = Smart2Pay_API_Plugin::VERSION;
			}
		...
		public static function add_user_fields( $user ) {
			echo self::render_template( 's2pa-settings/user-fields.php', array( 'user' => $user ) );
		}
		...
		public static function save_user_fields( $user_id ) {
			...
			update_user_meta( $user_id, 's2pa_user-example-field1', $user_fields[ 's2pa_user-example-field1' ] );
			update_user_meta( $user_id, 's2pa_user-example-field2', $user_fields[ 's2pa_user-example-field2' ] );
		}
		...
		public static function validate_user_fields( $user_id, $user_fields ) {
			if ( $user_fields[ 's2pa_user-example-field1' ] == false ) {
				$user_fields[ 's2pa_user-example-field1' ] = true;
				add_notice( 'Example Field 1 should be true', 'error' );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				$current_field2 = get_user_meta( $user_id, 's2pa_user-example-field2', true );

				if ( $current_field2 != $user_fields[ 's2pa_user-example-field2' ] ) {
					$user_fields[ 's2pa_user-example-field2' ] = $current_field2;
